...
![Version](https://img.shields.io/badge/Version-1.2.10--rc-yellow.svg)
...
- **Supports Actions:** Automates event handling by mapping HTML events to .NET Viewmodel's methods with optional parameter binding.
...
- **Auto-Generated Function Caching:** Initialization, update UI and action registration functions generated by Vitraux can be cached for faster load times.
...
1. **Add your C# code and a minimal configuration example**
   ...
   public record class PetOwner
   {
      ...
      public void ShowGreeting() => Console.WriteLine($"Hello {Name}!"); 
   }
   ...
   public class PetOwnerConfiguration : IModelConfiguration<PetOwner>
   {
      ...
      public ModelMappingData ConfigureMapping(IModelMapper<PetOwner> modelMapper)
         => modelMapper
               .MapAction(po => po.ShowGreeting()).FromInputs.ById("greet-button").On("click")
...
In addition to DOM updates, Vitraux also handles event wiring through **Actions**. During the build process, it generates JavaScript responsible for registering event listeners (such as *click*, *change*, etc.) on the mapped HTML elements. When those events fire, Vitraux routes them through an internal action dispatcher in WebAssembly, which invokes the corresponding .NET method — synchronously or asynchronously — and, when needed, gathers parameters automatically from inputs or via user-defined binders.
