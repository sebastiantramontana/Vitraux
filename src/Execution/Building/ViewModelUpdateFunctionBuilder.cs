...
internal class ViewModelUpdateFunctionBuilder<TViewModel>(
    ...
    IViewModelJsNamesRepositoryGeneric<TViewModel> viewModelJsNamesRepository,
    ...
    : IViewModelUpdateFunctionBuilder<TViewModel>
{
    public Task Build(string vmKey, ConfigurationBehavior configurationBehavior, ModelMappingData modelMappingData)
    {
        ...
        return configurationBehavior.AutoGeneratedFunctionCaching switch
        {
            AutoGeneratedFunctionNoCache => InvokeInitializationViewFunctionsNoCache(vmKey, fullObjNames, configurationBehavior.QueryElementStrategy),
            AutoGeneratedFunctionCacheByVersion cacheByVersion => InvokeInitializationViewFunctionsByVersion(vmKey, cacheByVersion.Version, fullObjNames, configurationBehavior.QueryElementStrategy),
            _ => notImplementedCaseGuard.ThrowException<Task>(configurationBehavior.AutoGeneratedFunctionCaching)
        };
    }

    private void StoreSerializationData(string vmKey, FullObjectNames fullObjectNames)
    {
        viewModelJsNamesRepository.ViewModelKey = vmKey;
        viewModelJsNamesRepository.ViewModelJsNames = encodedSerializationDataMapper.MapFromFull(fullObjectNames);
    }
